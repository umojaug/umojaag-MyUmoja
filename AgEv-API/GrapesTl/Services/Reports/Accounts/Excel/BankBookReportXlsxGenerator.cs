...
public class BankBookXlsxGenerator(IUnitOfWork unitOfWork) : IReportGenerator
{
    public string Key => "bankBookXlsx";
    private readonly IUnitOfWork _unitOfWork = unitOfWork;

    private async Task<List<BankBookDto>> GetDataAsync(ReportParams model)
    {
        ...
        var data = (await _unitOfWork.SP_Call.List<BankBookDto>("AcFisBankBookGetAll", parameter)).ToList();
        return data;
    }
    ...
    public async Task GenerateAsync(XLWorkbook workbook, ReportParams model)
    {
        ...
        worksheet.Cell("A2").Value = "Bank Book Report";
