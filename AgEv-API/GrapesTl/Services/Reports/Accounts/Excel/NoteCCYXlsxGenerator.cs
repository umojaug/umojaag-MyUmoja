...
public class NoteCCYXlsxGenerator(IUnitOfWork unitOfWork) : IReportGenerator
{
    public string Key => "noteCCYXlsx";
    ...
    private async Task<List<NoteCCYDto>> GetDataAsync(ReportParams model)
    {
        ...
        var data = (await _unitOfWork.SP_Call.List<NoteCCYDto>("AcFisNoteCcy", parameter)).ToList();
        return data;
    }
    ...
    public async Task GenerateAsync(XLWorkbook workbook, ReportParams model)
    {
        ...
        var worksheet = workbook.Worksheets.Add("NoteCCY Report");
        worksheet.Cell("A2").Value = "Note CCY Report";
