...
public class ProcessingIncomeBranchXlsxGenerator(IUnitOfWork unitOfWork) : IReportGenerator
{
    public string Key => "processingIncomeBranchXlsx";
    private readonly IUnitOfWork _unitOfWork = unitOfWork;

    private async Task<List<ProcessingIncomeBranchDto>> GetDataAsync(ReportParams model)
    {
        ...
        var data = (await _unitOfWork.SP_Call.List<ProcessingIncomeBranchDto>("AcFisProcessingIncomeBranch", parameter)).ToList();
        return data;
    }
    ...
    public async Task GenerateAsync(XLWorkbook workbook, ReportParams model)
    {
        ...
        worksheet.Cell("A2").Value = "Processing Income Branch";
        ...
        var columnsToInclude = new List<(string Field, string Group, string Header)>
        {
            ...
            // Disbursement Amount
            ("DisbursementAmountUML", "Disbursement Amount", "UML"),
            ("DisbursementAmountUBL", "Disbursement Amount", "UBL"),
            ("DisbursementAmountUAL", "Disbursement Amount", "UAL"),
            ("DisbursementAmountTotal", "Disbursement Amount", "TOTAL"),
        ...
        Type dtoType = typeof(ProcessingIncomeBranchDto);
