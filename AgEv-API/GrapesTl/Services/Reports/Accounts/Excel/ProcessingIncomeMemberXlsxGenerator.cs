...
public class ProcessingIncomeMemberXlsxGenerator(IUnitOfWork unitOfWork) : IReportGenerator
{
    public string Key => "processingIncomeMemberXlsx";
    private readonly IUnitOfWork _unitOfWork = unitOfWork;

    private async Task<List<ProcessingIncomeMemberDto>> GetDataAsync(ReportParams model)
    {
        ...
        var data = (await _unitOfWork.SP_Call.List<ProcessingIncomeMemberDto>("AcFisProcessingIncomeMember", parameter)).ToList();
        return data;
    }
    ...
    public async Task GenerateAsync(XLWorkbook workbook, ReportParams model)
    {
        ...
        worksheet.Cell("A2").Value = "Processing Income Member";
        ...
        var columnsToInclude = new List<(string Field, string Group, string Header)>
        {
            ...
            ("OpeningBalanceTotal", "Opening Balance", "Total"),
            ...
            // Amortized to P&L group
            ("AmortizedUML", "Amortized to P&L", "UML"),
            ("AmortizedUBL", "Amortized to P&L", "UBL"),
            ("AmortizedUAL", "Amortized to P&L", "UAL"),
            ("AmortizedTotal", "Amortized to P&L", "Total"),
            ...
            ("ClosingBalanceTotal", "Closing Balance", "Total")
        };


        // Create DataTable
        DataTable dataTable = new DataTable();
        Type dtoType = typeof(ProcessingIncomeMemberDto);
        ...
        if (dataTable.Columns.Contains("SerialNo"))
            totalRow["SerialNo"] = DBNull.Value;

        foreach (DataColumn column in dataTable.Columns)
        {
            if (column.ColumnName is "SerialNo" or "BranchName" || column.DataType == typeof(string))
                continue;
