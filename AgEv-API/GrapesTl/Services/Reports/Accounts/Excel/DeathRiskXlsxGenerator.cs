...
public class DeathRiskXlsxGenerator(IUnitOfWork unitOfWork) : IReportGenerator
{
    public string Key => "deathRiskXlsx";
    private readonly IUnitOfWork _unitOfWork = unitOfWork;

    private async Task<List<DeathRiskDto>> GetDataAsync(ReportParams model)
    {
        ...
        var data = (await _unitOfWork.SP_Call.List<DeathRiskDto>("AcFisDeathRisk", parameter)).ToList();
        return data;
    }
    ...
    public async Task GenerateAsync(XLWorkbook workbook, ReportParams model)
    {
        ...
        worksheet.Cell("A2").Value = "Death Risk";
        ...
        var columnsToInclude = new List<(string Field, string Group, string Header)>
        {
            ...
            ("DisbursedAmount", "", "Disbursed Amount (Principle Only)"),
            ("LoanOutstandingAmount", "", "Loan Outstanding Amount (Principle + Interest)"),
            ("SettlementAgainstDeath90Percent", "Settlement Against Death", "90%"),
            ("SettlementAgainstDeath10Percent", "Settlement Against Death", "10%"),
            ("TotalSettlementAgainstDeath", "Settlement Against Death", "Total"),
            ("Status", "", "Status")
        };
        ...
        Type dtoType = typeof(DeathRiskDto);
