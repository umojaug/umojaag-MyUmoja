...
public class AgingXlsxGenerator(IUnitOfWork unitOfWork) : IReportGenerator
{
    public string Key => "agingXlsx";
    private readonly IUnitOfWork _unitOfWork = unitOfWork;

    private async Task<List<AgingDto>> GetDataAsync(ReportParams model)
    {
        ...
        var data = (await _unitOfWork.SP_Call.List<AgingDto>("AcFisAgingReport", parameter)).ToList();
        return data;
    }
    ...
    public async Task GenerateAsync(XLWorkbook workbook, ReportParams model)
    {
        ...
        worksheet.Cell("A2").Value = "Aging Report Branch Wise";
        ...
        var columnsToInclude = new List<(string Field, string Group, string Header)>
        {
            ...
            ("Days", "", "Days"),
            ("ProvisionPercentage", "", "Provision Percentage"),
            ...
            ("PrincipalProvisionAmount", "", "Principal Provision Amount"),
            ("InterestProvisionAmount", "", "Interest Provision Amount"),
        };
        ...
        if (dataTable.Columns.Contains("Classifications"))
            totalRow["Classifications"] = "Total";
        if (dataTable.Columns.Contains("SL"))
            totalRow["SL"] = DBNull.Value;

        foreach (DataColumn column in dataTable.Columns)
        {
            if (column.ColumnName is "SL" or "Classifications" || column.DataType == typeof(string))
                continue;
