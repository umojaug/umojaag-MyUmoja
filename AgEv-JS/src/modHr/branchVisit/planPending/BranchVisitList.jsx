...
const BranchVisitList = () => {
  const {
    ...
    isError,
    refetch,
  } = useGetData("hrDepartment", "/branchVisit/pendinglist");
          ...
          list.data.map((item) => (
            ...
            >
              ...
              <div className="flex justify-end space-x-2">
                <AcceptButton
                  action={refetch}
                  path={`/branchVisit/approve/${item.branchVisitId}`}
                />
                <RejectRemarkButton
                  action={refetch}
                  path={`/branchVisit/reject/${item.branchVisitId}`}
                  noted="Yes"
                />
              </div>
            </div>
