...
const TransferVoucherEditModal = ({
  path,
  action,
  editData = [],
  transId,
  ...
  // Find Payment Type
  const creditEntry = items.find(item => item.entryType === "Cr");
  const paymentType = creditEntry ?
    (creditEntry.ledgerName.toLowerCase().includes("cash") ? "Cash" : "Bank") :
    "Cash";
  ...
  const addItem = async () => {
    ...
    try {
      ...
      const { id: transferToId, name: transferToName } = extractParts(
        formData.transferToId
      );

      const transferExists = items.some((item) => {
        return item.ledgerId === transferToId;
      });

      if (transferExists) {
        toast.error(`Transfer "${transferToName}" is already added to the list!`);
        return;
      }
  ...
  return (
    <>
      <button
        ...
        data-tooltip-content="Edit Transfer Voucher"
      >
        <MdEdit size={20} />
      </button>
...
export default TransferVoucherEditModal;