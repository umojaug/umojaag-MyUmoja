...
const schema = yup.object({
  receiveId: yup.string().required("Required."),
  amount: yup
    .number()
...
const ReceiveVoucherEditModal = ({
  ...
  const { receiveId, amount, particulars } = errors;
  ...
  const addItem = async () => {
    ...
    try {
      ...
      const { id: receiveIdExtracted, name: receiveName } = extractParts(
        formData.receiveId
      );
      const receiveExists = items.some((item) => {
        return item.ledgerId === receiveIdExtracted;
      });

      if (receiveExists) {
        toast.error(`Receive "${receiveName}" is already added to the list!`);
        return;
      }
      ...
      const creditItem = {
        id: timestamp + 1,
        ledgerId: receiveIdExtracted,
        ledgerName: receiveName,
        dr: 0,
        cr: formatedAmount,
      ...
      setItems((prevItems) => {
        const newItems = [...prevItems, creditItem];

        const totalCr = newItems
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                        <div>
                          <Select2FromDbWithName
                            control={control}
                            label="Select Receive From"
                            path="/acLedger/SelectByReceive"
                            name="receiveId"
                            errorMessage={receiveId?.message}
                          />
                        </div>
                        <div>
                          <InputNumber
...
export default ReceiveVoucherEditModal;
