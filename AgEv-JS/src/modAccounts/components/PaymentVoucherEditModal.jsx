...
const PaymentVoucherEditModal = ({
  ...
  const { paymentId, amount, particulars } = errors;
  ...
  const addItem = async () => {
    ...
    try {
      ...
      const { id: paymentIdExtracted, name: paymentName } = extractParts(
        formData.paymentId
      );
      const paymentExists = items.some((item) => {
        return item.ledgerId === paymentIdExtracted;
      });

      if (paymentExists) {
        toast.error(`Payment "${paymentName}" is already added to the list!`);
        return;
      }
      ...
      const debitItem = {
        id: timestamp + 1,
        ledgerId: paymentIdExtracted,
        ledgerName: paymentName,
        dr: formatedAmount,
        cr: 0,
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                        <div>
                          <Select2FromDbWithName
                            control={control}
                            label="Select Payment To"
                            path="/acLedger/SelectByPayment"
                            name="paymentId"
                            errorMessage={paymentId?.message}
                          />
                        </div>
                        <div>
                          <InputNumber
...
export default PaymentVoucherEditModal;
